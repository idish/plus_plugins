...
internal class Share(
    ...
    fun startActionViewIntent(uriString: String, packageName: String) {
        val uri = Uri.parse(uriString)
        val intent = Intent(Intent.ACTION_VIEW, uri)
        intent.setPackage(packageName)

        startActivity(intent, false)
//        //Check that the user can open the intent, meaning they have the app installed
//        if (activity?.packageManager?.resolveActivity(
//                intent,
//                0
//            ) != null
//        ) startActivity(intent, false) else throw IOException("No intent found to share")
    }
    ...
    fun shareFiles(
        ...
        resInfoList.forEach { resolveInfo ->
            val resolvedInfoPackageName = resolveInfo.activityInfo.packageName
